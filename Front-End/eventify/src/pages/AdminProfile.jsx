...
export default function AdminProfile() {
  ...
  useEffect(() => {
          const fetchData = async () => {
              try {
                  ...
                  const response = await fetch(`http://localhost:8080/admin/getAdminByUsername/?adminUsername=${usernameExists}`);
  ...
  if (!items) return <p>Admin not found</p>;
 
   const goToUpdate = () => {
    navigate("/UpdateAdmin", { state: { items } });
  };
      //User Profile
  return (
                  ...
                  <MDBCardBody className="p-4">
                    ...
                    <MDBRow className="pt-1">
                      <MDBCol size="6" className="mb-3">
                        ...
                        <MDBCardText className="text-muted">{items.firstName}</MDBCardText>
                        <MDBTypography tag="h6">Surname</MDBTypography>
                        <MDBCardText className="text-muted">{items.lastName}</MDBCardText>
